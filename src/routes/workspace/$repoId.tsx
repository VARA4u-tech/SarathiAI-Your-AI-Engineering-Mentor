...
import { 
  FolderGit2, Shield, Users, ShoppingCart, CreditCard, 
  Package, Settings, Bell, Database, Route as RouteIcon,
  Search, Terminal, Play, Bug, FileText, LayoutTemplate,
  MessageSquare, Zap, Clock, AlertTriangle, Layers, BrainCircuit
} from "lucide-react";
  ...
  return (
    <main className="h-screen bg-background text-foreground flex overflow-hidden">
      ...
      <aside className="relative z-10 w-16 border-r border-border flex flex-col items-center py-4 glass shrink-0">
        <Link to="/dashboard" className="size-8 rounded-full bg-gradient-to-br from-fuchsia-400 to-cyan-400 shadow-[0_0_12px_currentColor] mb-8" />
        
        <div className="flex flex-col gap-4">
          <NavIcon icon={Search} tooltip="Understand" active={activeTab === "Understand"} onClick={() => setActiveTab("Understand")} />
          <NavIcon icon={LayoutTemplate} tooltip="Planner" active={activeTab === "Planner"} onClick={() => setActiveTab("Planner")} />
          <NavIcon icon={Terminal} tooltip="Build" active={activeTab === "Build"} onClick={() => setActiveTab("Build")} />
          <NavIcon icon={Bug} tooltip="Debug" active={activeTab === "Debug"} onClick={() => setActiveTab("Debug")} />
          <NavIcon icon={FileText} tooltip="Document" active={activeTab === "Document"} onClick={() => setActiveTab("Document")} />
        </div>
      </aside>
      ...
      <aside className="relative z-10 w-72 border-r border-border flex flex-col glass shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-6">
          <div>
            <div className="text-[10px] font-medium text-muted-foreground uppercase tracking-widest px-2 mb-2">Business Modules</div>
          ...
          <div>
            <div className="text-[10px] font-medium text-muted-foreground uppercase tracking-widest px-2 mb-2">System</div>
            <div className="space-y-0.5">
      ...
      <aside className="relative z-10 w-80 border-l border-border flex flex-col glass shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          <div>
            <h4 className="text-xs uppercase tracking-widest text-muted-foreground mb-3">Selected Files</h4>
          ...
          <div>
            <h4 className="text-xs uppercase tracking-widest text-muted-foreground mb-3">Architecture Dependencies</h4>
            <div className="p-3 rounded-xl bg-foreground/5 border border-border text-xs text-muted-foreground space-y-2">
              <div className="flex justify-between"><span>PaymentService</span> <span className="text-foreground">Required</span></div>
              <div className="flex justify-between"><span>EmailService</span> <span className="text-foreground">Optional</span></div>
              <div className="flex justify-between"><span>InventoryDb</span> <span className="text-foreground">Required</span></div>
            </div>
          </div>
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground leading-relaxed p-3 rounded-xl bg-foreground/5 border border-border">
              "You previously asked to optimize the inventory check query. I've stored that context for when we edit OrderService.ts."
            </p>
          </div>
        </div>
      </aside>
...
function ModuleItem({ icon: Icon, label, active }: any) {
  return (
    <button className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-colors ${
      active ? "bg-fuchsia-400/10 text-fuchsia-400" : "text-muted-foreground hover:bg-foreground/5 hover:text-foreground"
    }`}>
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="glass p-8 rounded-3xl border border-border">
        ...
        <p className="text-sm text-muted-foreground mb-6">Generated implementation plan based on architecture analysis.</p>
        
        <div className="grid grid-cols-3 gap-4 mb-8">
          <div className="p-4 rounded-2xl bg-background border border-border">
            <div className="text-xs text-muted-foreground uppercase tracking-widest mb-1">Complexity</div>
            <div className="text-lg font-medium text-fuchsia-400">High</div>
          </div>
          <div className="p-4 rounded-2xl bg-background border border-border">
            <div className="text-xs text-muted-foreground uppercase tracking-widest mb-1">Est. Time</div>
            <div className="text-lg font-medium flex items-center gap-2"><Clock className="size-4"/> 4 hours</div>
          </div>
          <div className="p-4 rounded-2xl bg-background border border-border">
            <div className="text-xs text-muted-foreground uppercase tracking-widest mb-1">Risk Level</div>
            <div className="text-lg font-medium text-orange-400 flex items-center gap-2"><AlertTriangle className="size-4"/> Medium</div>
          </div>
        </div>

        <div className="space-y-6">
          <div>
            <h3 className="text-sm font-medium mb-3 flex items-center gap-2"><Layers className="size-4 text-cyan-400"/> Affected Systems</h3>
            <div className="flex gap-2 flex-wrap text-xs">
              <span className="px-3 py-1 rounded-full bg-foreground/10 border border-border">Backend (2 files)</span>
              <span className="px-3 py-1 rounded-full bg-foreground/10 border border-border">Database (1 migration)</span>
              <span className="px-3 py-1 rounded-full bg-foreground/10 border border-border">API (1 new route)</span>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              <div className="p-4 rounded-xl border border-border bg-background/50">
                ...
                <div className="text-xs text-muted-foreground mt-1">Add `refundStatus` and `refundAmount` to Order model.</div>
              </div>
              <div className="p-4 rounded-xl border border-border bg-background/50">
                <div className="text-sm font-medium">2. Create Stripe Webhook Handler</div>
                <div className="text-xs text-muted-foreground mt-1">Listen for `charge.refunded` events in PaymentService.</div>
              </div>
              <div className="p-4 rounded-xl border border-border bg-background/50">
                <div className="text-sm font-medium">3. Implement Refund API Route</div>
                <div className="text-xs text-muted-foreground mt-1">POST `/api/orders/:id/refund` requiring admin privileges.</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-6 pb-6">
        ...
        <div className="flex justify-start">
          <div className="max-w-[90%] rounded-2xl rounded-tl-sm glass px-5 py-4 text-sm border border-border space-y-4 shadow-xl">
            <p>
              Based on my analysis of <span className="font-mono text-xs text-fuchsia-400 bg-fuchsia-400/10 px-1 rounded">OrderService.ts</span>, the current checkout flow uses a pessimistic lock on the database level:
            </p>
            <div className="rounded-lg bg-background p-4 border border-border font-mono text-xs text-muted-foreground overflow-x-auto">
              <div className="text-foreground">await prisma.$transaction(async (tx) ={">"} {'{'}</div>
              <div className="pl-4">// 1. Lock rows</div>
              <div className="pl-4">await tx.$executeRaw`SELECT * FROM Inventory WHERE id IN (...) FOR UPDATE`;</div>
              <div className="pl-4 mt-2">// 2. Check quantities</div>
              <div className="pl-4 mt-2">// 3. Deduct if sufficient</div>
              <div>{'}'})</div>
            </div>
            <p>
              This is safe but can cause bottlenecks during high-traffic drops. Would you like me to generate a Redis-based optimistic locking approach?
            </p>
          </div>
        </div>
      </div>
