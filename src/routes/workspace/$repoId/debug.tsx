...
import { 
  Bug, ArrowRight, UploadCloud, Terminal, 
  CheckCircle2, AlertTriangle, Play, FileCode, Copy
} from "lucide-react";
  ...
  return (
    <main className="h-screen bg-background text-foreground flex flex-col overflow-hidden">
      ...
      <header className="relative z-10 h-16 border-b border-border flex items-center px-6 glass shrink-0">
        <Link to="/workspace/$repoId" params={{ repoId }} className="text-muted-foreground hover:text-foreground transition flex items-center gap-2 text-sm font-medium">
        ...
        <div className="max-w-5xl mx-auto grid md:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Paste error logs, stack traces, or describe the bug.</p>
            </div>
    ...
    at runMicrotasks (<anonymous>)"
              ...
              <div className="flex gap-4">
                <button type="button" className="flex-1 glass border border-border border-dashed hover:border-foreground/40 rounded-xl p-4 flex flex-col items-center justify-center gap-2 text-sm text-muted-foreground transition">
              ...
              >
                {analyzing ? (
                  <span className="flex items-center gap-2 animate-pulse"><Bug className="size-4" /> Analyzing Codebase...</span>
                ) : (
                  <span className="flex items-center gap-2"><Play className="size-4" /> Analyze Bug</span>
                )}
              </button>
            </form>
          </div>
          ...
          <div className="h-full">
            {!result && !analyzing ? (
              <div className="h-full border border-border border-dashed rounded-3xl flex items-center justify-center p-8 opacity-50 glass">
                <p className="text-sm text-muted-foreground text-center">Awaiting stack trace to analyze root cause.</p>
              </div>
            ) : analyzing ? (
              <div className="h-full rounded-3xl flex flex-col items-center justify-center p-8 glass border border-border">
                ...
                <p className="text-xs text-muted-foreground mt-2 text-center max-w-xs">Scanning 432 files to find the exact origin of the null pointer exception.</p>
              </div>
            ) : (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                ...
                <div className="glass rounded-3xl p-6 border border-border shadow-xl">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className="text-xs font-mono bg-foreground/10 px-2 py-1 rounded">Confidence: 98%</span>
                  </div>
                  
                  <p className="text-sm text-muted-foreground mb-4 leading-relaxed">
                    The <code className="text-foreground bg-background px-1 rounded">user</code> object returned from <code className="text-foreground bg-background px-1 rounded">AuthService.validate()</code> can be undefined if the token has expired, but <code className="text-foreground bg-background px-1 rounded">OrderService.ts</code> assumes it is always defined before accessing <code className="text-foreground bg-background px-1 rounded">user.id</code>.
                  </p>
                  ...
                  <div className="p-3 bg-background rounded-xl border border-border text-xs font-mono text-muted-foreground mb-4">
                    ...
                    <div className="line-through opacity-50 text-red-400">- const userId = req.user.id;</div>
                    <div className="text-emerald-400">+ const userId = req.user?.id;</div>
                    <div className="text-emerald-400">+ if (!userId) throw new UnauthorizedError();</div>
                  </div>
                ...
                <div className="glass rounded-2xl p-6 border border-border">
                  ...
                  <ul className="text-sm space-y-2">
                    <li className="flex justify-between items-center"><span className="text-muted-foreground">CartController.ts</span> <span className="text-orange-400 text-xs">Similar Pattern</span></li>
                    <li className="flex justify-between items-center"><span className="text-muted-foreground">ProfileService.ts</span> <span className="text-emerald-400 text-xs">Safe</span></li>
                  </ul>
                </div>
              </motion.div>
            )}
