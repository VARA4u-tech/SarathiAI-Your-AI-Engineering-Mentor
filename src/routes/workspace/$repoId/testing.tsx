...
import { 
  TestTube2, ArrowRight, CheckCircle2, PlayCircle, 
  TerminalSquare, FileCode2, Download
} from "lucide-react";
        ...
        <div className="max-w-6xl mx-auto space-y-8">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div className="glass rounded-3xl p-6 border border-border">
              <h3 className="text-xs uppercase tracking-widest text-muted-foreground mb-4">Total Coverage</h3>
            ...
            <div className="md:col-span-2 glass rounded-3xl p-6 border border-border">
              <h3 className="text-xs uppercase tracking-widest text-muted-foreground mb-4">AI Test Generator</h3>
              <div className="flex gap-4">
                <div className="flex-1 bg-background/50 border border-border rounded-xl p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-xs bg-orange-400/10 text-orange-400 px-2 py-0.5 rounded">Low Coverage (42%)</span>
                  </div>
                  <p className="text-xs text-muted-foreground mb-4">Missing edge cases for concurrent refunds and invalid coupon codes.</p>
                ...
                <div className="flex-1 bg-background/50 border border-border rounded-xl p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-xs bg-fuchsia-400/10 text-fuchsia-400 px-2 py-0.5 rounded">Uncovered</span>
                  </div>
                  <p className="text-xs text-muted-foreground mb-4">Missing integration tests for Stripe webhook signature validation.</p>
                  <button className="w-full rounded-lg glass border border-border py-2 text-xs font-medium hover:bg-foreground/5 transition flex justify-center items-center gap-2">
...
function TestSuite({ name, tests, passed, failed = 0, time, type }: any) {
  ...
  return (
    <div className="glass rounded-2xl p-4 border border-border flex items-center justify-between transition hover:border-foreground/30">
      <div className="flex items-center gap-4">
        <div className={`size-10 rounded-xl flex items-center justify-center ${allPassed ? 'bg-emerald-400/10 text-emerald-400' : 'bg-red-400/10 text-red-400'}`}>
          {allPassed ? <CheckCircle2 className="size-5" /> : <TestTube2 className="size-5" />}
        </div>
        <div>
          <h4 className="text-sm font-medium flex items-center gap-2">
            {name}
            <span className="text-[10px] uppercase tracking-widest text-muted-foreground bg-foreground/5 px-2 py-0.5 rounded">{type}</span>
          </h4>
