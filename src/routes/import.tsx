      ...
      <div className="relative z-10 w-full max-w-4xl grid md:grid-cols-2 gap-8 items-start">
        ...
        <div>
          ...
          <p className="text-muted-foreground mb-10 text-sm max-w-sm">
            Connect your codebase. CodePilot AI will ingest, analyze, and build a knowledge graph of your architecture.
          </p>
          ...
          <form onSubmit={handleValidate} className="space-y-6">
            <div className="glass p-1 rounded-full flex items-center pr-2 border-border focus-within:border-foreground/40 transition">
              <div className="pl-5 pr-3 text-muted-foreground"><Github className="size-5" /></div>
              <input 
                type="text" 
                value={url}
            ...
            <button type="button" className="w-full glass rounded-2xl p-8 border border-border border-dashed hover:border-foreground/30 transition flex flex-col items-center justify-center gap-3 group">
        ...
        <div className="h-full pt-12 md:pt-28">
          {stats ? (
            ...
            >
              ...
              <Link to="/processing" className="block w-full text-center rounded-full bg-foreground text-background px-6 py-4 text-sm font-medium hover:opacity-90 transition">
                Start Analysis Pipeline →
              </Link>
            </motion.div>
          ) : (
            <div className="h-full border border-border border-dashed rounded-3xl flex items-center justify-center p-8 opacity-50">
              <p className="text-sm text-muted-foreground text-center">Connect a repository to see validation statistics.</p>
            </div>
          )}
        </div>
...
function StatRow({ label, val, highlight = false }: { label: string, val: string | number, highlight?: boolean }) {
  return (
    <div className="flex items-center justify-between text-sm">
      <span className="text-muted-foreground">{label}</span>
      <span className={highlight ? "text-fuchsia-400 font-medium" : "text-foreground font-medium"}>{val}</span>
    </div>
  );
}
