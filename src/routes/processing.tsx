      ...
      <div className="relative z-10 w-full max-w-xl p-6">
        <div className="text-center mb-12">
          ...
          <p className="text-muted-foreground text-sm">
            {isComplete
              ? "Your knowledge graph and vector embeddings are ready."
              : "Please wait while CodePilot AI ingests your codebase..."}
          </p>
        </div>
            ...
            {pipelineSteps.map((step, index) => {
              ...
              return (
                <div key={step} className="flex items-center gap-4">
                  <div
                    className={`relative z-10 size-5 rounded-full flex items-center justify-center transition-colors duration-500 ${
                      status === "done"
                        ? "bg-emerald-400 text-background"
                        : status === "active"
                          ? "bg-fuchsia-400 text-background animate-pulse"
        ...
        >
          <Link
            to="/workspace/$repoId"
            params={{ repoId: "demo-repo" }}
            className="rounded-full bg-foreground text-background px-8 py-4 text-sm font-medium hover:opacity-90 transition flex items-center gap-2"
          >
